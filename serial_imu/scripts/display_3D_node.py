...
# Main scene
scene=display(title="Hipnuc IMU Main Screen")
...
scene2 = display(title='Hipnuc IMU Roll, Pitch, Yaw',x=550, y=0, width=500, height=500,center=(0,0,0), background=(0,0,0))
scene2.range=(1,1,1)
scene2.select()
#Roll, Pitch, Yaw
...
# IMU object
platform    = box(length = 1.0, height = 0.05, width = 0.65, color = color.blue, up = (0, 0, 1), axis = (-1, 0, 0))
plat_line_x = box(length = 1.1, height = 0.08, width = 0.1, color = color.red, up = (0, 0, 1), axis = (-1, 0, 0))
plat_line_y = box(length = 0.8, height = 0.08, width = 0.1, color = color.yellow, up = (0, 1, 0), axis = (0, -1, 0))

plat_arrow_x = arrow(length = -0.8, color = color.red, up = (0, 0, 1), axis = (-1, 0, 0), shaftwidth = 0.06, fixedwidth = 1)
plat_arrow_y = arrow(length = 0.4, color = color.yellow, up = (0, -1, 0), axis = (0, 1, 0), shafrwidth = 0.06, fixedwidth = 1)
plat_arrow_z = arrow(length = 0.4, color = color.cyan, up = (-1, 0, 0), axis = (0, 0, 1), shaftwidth = 0.06, fixedwidth = 1)

rospy.init_node("display_3D_node")
...
def processIMU_message(imuMsg):
    ...
    axis_x = (cos(yaw) * cos(pitch), cos(pitch) * sin(yaw), -sin(pitch))
    axis_y = (cos(yaw) * sin(roll) * sin(pitch) - cos(roll) * sin(yaw), sin(roll) * sin(pitch) * sin(yaw) + cos(roll) * cos(yaw), sin(roll) * cos(pitch))
    axis_z = (cos(roll) * sin(pitch) * cos(yaw) + sin(roll) * sin(yaw), cos(roll) * sin(pitch) * sin(yaw) - sin(roll) * cos(yaw), cos(roll) * cos(pitch))

    platform.axis        = axis_x
    platform.up          = axis_z
    platform.length      = 1.0
    plat_line_x.axis     = axis_x
    plat_line_x.up       = axis_z
    plat_line_x.length   = 1.1
    plat_line_y.axis     = axis_y
    plat_line_y.up       = axis_z
    plat_line_y.length   = 0.7

    plat_arrow_x.up      = axis_z
    plat_arrow_x.axis    = axis_x
    plat_arrow_x.length  = 0.8
    plat_arrow_y.up      = axis_z
    plat_arrow_y.axis    = axis_y
    plat_arrow_y.length  = 0.6
    plat_arrow_z.axis    = axis_z
    plat_arrow_z.up      = axis_x
    plat_arrow_z.length  = 0.4
