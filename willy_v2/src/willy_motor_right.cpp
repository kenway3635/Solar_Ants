...
willy_motor::willy_motor() : ID_1(0x01), ID_2(0x02), FC(0x10), cmd_num(0x0002), cmd_numx2(0x04), Gear_Ratio(50), Wheel_Radius(0.075), Wheel_Base(0.255),  last_dir(0), vel(0),is_motor_set(true), C35(3000)  {
	...
	motor_pub = n.advertise<std_msgs::Int32>("/motor_vr", 20);
	cmd_stop=n.subscribe("/cmd_stop",1,&willy_motor::Stop, this);
	cmd_sub = n.subscribe("cmd_motor", 1, &willy_motor::callback, this);

	// temp_pub = n.advertise<std_msgs::Int32>("/vr_temp", 20);
	// cmd_pub = n.advertise<std_msgs::Int32>("/vr_cmd", 20);
	// load_pub = n.advertise<std_msgs::Int32>("/vr_load", 20);
...
void willy_motor::run()
{
    ...
    while(ros::ok()) 
	{
		...
		if (last_cmd_counter != cmd_counter) 
		{
			...
			// vel = (int)(VR * Gear_Ratio * rad2rev / (sec2min * Wheel_Radius));
...
int willy_motor::calculate_cmd(int v)
{
	...
	if (stop_r)//emergency stop
	{
		if (last_dir == 1)
		{
			send_cmd_single(ID_2, 0x007D, 0b0000000000101010);
		}
		else
		{
			send_cmd_single(ID_2, 0x007D, 0b0000000000001010);
		}
		vel_data=0;
	}
	else
	...
	if (v > lower_lim) {
		...
		if (last_dir != 1) send_cmd_single(ID_2, 0x007D, 0b0000000000111010);
		last_dir = 1;
	}
	else if (abs(v) < lower_lim) {
		vel_data = 0;
		if (last_dir == 1) send_cmd_single(ID_2, 0x007D, 0b0000000000110010);
		else if (last_dir == -1) send_cmd_single(ID_2, 0x007D, 0b0000000000010010);
		last_dir=0;
	}
	else if(v<-lower_lim) 
	{
		...
		if (last_dir != -1) send_cmd_single(ID_2, 0x007D, 0b0000000000011010);
		last_dir = -1;
	}
	}
	return vel_data;
