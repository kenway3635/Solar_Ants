...
class RosToMqttBridge(Bridge):
    ...
    def _publish(self, msg: rospy.Message):
        ...
        #payload = self._serialize(extract_values(msg)['data'])
...
class MqttToRosBridge(Bridge):
    ...
    def _create_ros_message(self, mqtt_msg: mqtt.MQTTMessage) -> rospy.Message:
        ...
        if self._serialize.__name__ == "packb":
            print(type(mqtt_msg))
            msg_dict = self._deserialize(self._serialize({'data':mqtt_msg.payload}), raw=False)
            #msg_dict['data'] = str(msg_dict['data'],encoding="utf-8")
            msg_dict['data'] = bytes.decode(msg_dict['data'] ) 
            #print(msg_dict)
            #print(type(msg_dict))
