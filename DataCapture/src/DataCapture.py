...
if __name__ == '__main__':
  ...
  save_path = 'cnn-odom-rnn.ckpt'
  model.load_state_dict(torch.load(save_path))
  model.eval()
  batch_size = 64
  data=np.zeros((batch_size,49157))
  ...
  while not rospy.is_shutdown():
    ...
    #frame_1d = np.random.randint(10,size = 49157)
    ...
    input = torch.FloatTensor(data[:,:-2]).to(device)
    predict = model(input)
    #print('value =', predict[9])
    time=time+1
    if time >=batch_size:
